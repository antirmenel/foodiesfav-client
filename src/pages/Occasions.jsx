...
function Occasions() {
  ...
  const occasionsRecipes = allRecipes.filter(
    (recipe) => recipe.type === "special occasion"
  );
  const filteredRecipes = occasionsRecipes.filter((recipe) => {
  ...
  return (
    <div className="dark:bg-dark dark:text-white">
      ...
      >
        <motion.h1
          className="text-3xl font-bold mb-6 text-center Bodoni uppercase text-orange-500 tracking-widest"
        ...
        >
          {filteredRecipes.map((recipe) => (
            <Link
              key={recipe.id}
              to={`/recipe/${recipe.id}`}
              className="bg-white shadow-md rounded-lg overflow-hidden h-full dark:bg-dark dark:text-white"
